...
public class BookingCTL {
	...
	private static enum State {
		PHONE, ROOM, REGISTER, TIMES, CREDIT, APPROVED, CANCELLED, COMPLETED
	}
	...
	public void bookingTimesEntered(Date arrivalDate, int stayLength) {
		...
		if (room == null) {
			...
			String notAvailableStr = String.format("\n%s is not available betweenbetween %s and %s\n",
					selectedRoomType.getDescription(), format.format(arrivalDate), format.format(departureDate));
	...
	public void creditDetailsEntered(CreditCardType type, int number, int ccv) {
		...
		boolean approved = new CreditAuthorizer().authorize(creditCard, cost);
		if (approved) {
			...
			bookingUI.displayConfirmedBooking(room.getDescription(), room.getId(), arrivalDate, stayLength,
					guest.getName(), creditCard.getVendor(), number, cost, confirmationNumber);
